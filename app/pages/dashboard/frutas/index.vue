...
const { data, status, refresh } = await useAsyncData('frutas', () => fetchWithAuth<Fruta[]>('frutas'), {
  lazy: true,
  default: () => []
})

// Acciones por fila
function getRowItems(row: Row<Fruta>) {
  const fruta = row.original
  return [
    {
      type: 'label',
      label: 'Acciones'
    ...
    {
      label: fruta.estado ? 'Desactivar' : 'Activar',
      icon: fruta.estado ? 'i-lucide-trash' : 'i-lucide-refresh-cw',
      color: fruta.estado ? 'error' : 'success',
      onSelect: async () => {
        if (fruta.estado) {
          try {
            await fetchWithAuth(`frutas/${fruta.id_fruta}`, { method: 'DELETE' })
            toast.add({ title: 'Fruta desactivada', color: 'success' })
            refresh()
          } catch (error: any) {
...
// Columnas
const columns: TableColumn<Fruta>[] = [
  {
    id: 'select',
    header: ({ table }) =>
      h(UCheckbox, {
...
const selectedFruta = ref<Fruta | null>(null)

function openEditModal(fruta?: Fruta) {
  selectedFruta.value = fruta || null
...
const deleteMultiple = async () => {
  ...
  const ids = selectedRows.map(row => row.original.id_fruta)
  for (const id of ids) {
    await fetchWithAuth(`frutas/${id}`, { method: 'DELETE' })
  }
  toast.add({ title: `${ids.length} fruta(s) desactivada(s)`, color: 'success' })
  refresh()
}
</script>
...
<template>
  <UDashboardPanel>
    <template #header>
      <UDashboardNavbar title="Frutas">
        <template #leading>
          <UDashboardSidebarCollapse />
        </template>
        <template #right>
          <FrutaModal @saved="refresh" />
        </template>
      </UDashboardNavbar>
    </template>
